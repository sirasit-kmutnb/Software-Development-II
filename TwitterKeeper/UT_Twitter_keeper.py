...
class TestPullTweetsData(unittest.TestCase):
    ...
    def test_getAccessToAPI(self):
        self.assertEqual(self.pull_tweets._PullTweetsData__auth.consumer_key, self.api_key)
        self.assertEqual(self.pull_tweets._PullTweetsData__auth.consumer_secret, self.api_key_secret)

    def test_setUserAuthentication(self):
        self.assertEqual(self.pull_tweets._PullTweetsData__auth.access_token, self.access_token)
        self.assertEqual(self.pull_tweets._PullTweetsData__auth.access_token_secret, self.access_token_secret)

    def test_getTwitterAPI(self):
        self.assertIsInstance(self.pull_tweets._PullTweetsData__api, tweepy.API)

    def test_getHashtag(self):
        entity_hashtag = [{"text": "test1"},{"text": "test2"}]
        expected_hashtag = "#test1#test2"
        self.assertEqual(self.pull_tweets.getHashtag(entity_hashtag), expected_hashtag)
    ...
    def test_createDictData(self):
        ...
        self.assertEqual(self.pull_tweets.createDictData(
            tweet_author, tweet_create_at, hashtag, keyword, text), expected_tweet)
